...
<content>
    ...
    <script type="text/javascript">
        ...
        ref.refresh = function(text) {
            ...
            for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
                if (wl_sunit(uidx) < 0) E("noise" + uidx).innerHTML = wlnoise[uidx];
            }
        }
        ...
        dg.populate = function() {
            ...
            for (i = wldev.length - 1; i >= 0; --i) {
                ...
                if ((a[3] > 1000) || (a[4] > 1000)) e.txrx = ((a[3] > 1000) ? Math.round(a[3] / 1000) : '-') + ' / ' + ((a[4] > 1000) ? Math.round(a[4] / 1000) : '-'); //+ '<br><small>Mbps</small>';
            }
            ...
            for (i = list.length - 1; i >= 0; --i) {
                ...
                b = e.mac;
                if (e.mac.match(/^(..):(..):(..)/)) {
                    b += '<br /><a class="btn btn-small" href="http://standards.ieee.org/cgi-bin/ouisearch?' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="网卡查询">查询</a> ' + '<a class="btn btn-small" href="javascript:addStatic(' + i + ')" title="静态分配...">静态</a> ' + '<a class="btn btn-small" href="javascript:addqoslimit(' + i + ')" title="QoS带宽限制">限速</a>';

                    if (e.rssi != '') {
                        b += ' <a class="btn btn-small" href="javascript:addWF(' + i + ')" title="无线黑名单...">拉黑</a>';
                    }
                } else {
                    b = '';
                }
                ...
                if (e.qual) {
                    ...
                    this.insert( - 1, e, [e.ifname, b, (e.ip == '-') ? '': e.ip + '<br />' + e.name, (e.rssi != 0) ? e.rssi + ' <small>dBm</small>': '-', (e.qual < 0) ? '-': '<small>' + e.qual + '%</small> <div class="progress small"><div title="' + e.qual + '%" class="bar ' + bar + '" style="width: ' + e.qual + '%;"></div></div>', e.txrx, e.lease], false);
                }
            }
        }
        ...
        dg.setup = function() {
            ...
            this.headerSet(['接口', 'MAC地址', 'IP地址 (主机名)', 'RSSI', '信号质量 (%)', 'TX/RX 速率', '租期']);
            this.populate();
            this.sort(2);
        }
    ...
    <style>
        #dev-grid td.co5, #dev-grid td.co4 { text-align: center; }
    </style>
    <div class="box">
        <div class="heading">
            设备列表
            ...
            <script type="text/javascript">
                ...
                for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
                    ...
                    if (nvram['wl' + u + '_radio'] == '1') {
                        if (wl_sunit(uidx) < 0) {
                            var a = '';
                            if ((nvram['wl' + u + '_mode'] == 'ap') || (nvram['wl' + u + '_mode'] == 'wds')) a = '&nbsp; &nbsp; <button type="button" class="btn btn-primary" value="Measure" onclick="javascript:window.location=\'wlmnoise.cgi?_http_id=' + nvram.http_id + '&_wl_unit=' + u + '\'">\
测量 <i class="icon-signal"></i></button>';
                            f.push({
                                title: '<b>干扰基数 (' + wl_ifaces[uidx][0] + ')</b>',
    ...
    <script type="text/javascript">
        $('.box').after(genStdRefresh(1, 1, 'ref.toggle()'));
    </script>
    <script type="text/javascript">
        earlyInit();
        init();
