<!-- Tomato GUI Copyright (C) 2006-2008 Jonathan Zarate http://www.polarcloud.com/tomato/
...
<content>
    <script type="text/javascript" src="js/vpn.js">
    </script>
    <script type="text/javascript">
        ...
        sections = [['basic', '<i class="icon-system"></i> 基本设置'], ['advanced', '<i class="icon-shield"></i> 高级设置'], ['keys', '<i class="icon-lock"></i> 安全设置'], ['status', '<i class="icon-info"></i> 状态']];
        ccdTables = [];
        usersTables = [];
        statusUpdaters = [];
        ...
        function toggle(service, isup) {
            if (changed && !confirm("未保存的更改将丢失。 仍然继续?")) return;
        ...
        function verifyFields(focused, quiet) {
            ...
            if (focused) {
                ...
                if (serverindex >= 0) {
                    ...
                    if (focused.name.indexOf("_c2c") >= 0) ccdTables[servernumber - 1].reDraw();
                }
            }
            ...
            for (i = 0; i < tabs.length; ++i) {
                ...
                elem.display(PR('_vpn_' + t + '_ca'), PR('_vpn_' + t + '_crt'), PR('_vpn_' + t + '_dh'), PR('_vpn_' + t + '_key'), PR('_vpn_' + t + '_hmac'), PR('_f_vpn_' + t + '_rgw'), PR('_vpn_' + t + '_reneg'), auth.value == "tls");
            ...
            if (nvram.lan_ifname.length < 1) bridge1.options[0].disabled = true;
            if (nvram.lan1_ifname.length < 1) bridge1.options[1].disabled = true;
            if (nvram.lan2_ifname.length < 1) bridge1.options[2].disabled = true;
            if (nvram.lan3_ifname.length < 1) bridge1.options[3].disabled = true;

            var bridge2 = E('_vpn_server2_br');
            if (nvram.lan_ifname.length < 1) bridge2.options[0].disabled = true;
            if (nvram.lan1_ifname.length < 1) bridge2.options[1].disabled = true;
            if (nvram.lan2_ifname.length < 1) bridge2.options[2].disabled = true;
            if (nvram.lan3_ifname.length < 1) bridge2.options[3].disabled = true;
        ...
        CCDGrid.prototype.verifyFields = function(row, quiet) {
            ...
            if (f[1].value == "") {
                ferror.set(f[1], "公共名称是必需的。", quiet);
                ret = 0;
            }
            if (f[1].value.indexOf('>') >= 0 || f[1].value.indexOf('<') >= 0) {
                ferror.set(f[1], "公用名不能包含 '<' 或 '>' 字符。", quiet);
                ret = 0;
            }
            if (f[2].value != "" && !v_ip(f[2], quiet, 0)) ret = 0;
            if (f[3].value != "" && !v_netmask(f[3], quiet)) ret = 0;
            if (f[2].value == "" && f[3].value != "") {
                ferror.set(f[2], "必须提供子网和网络掩码，或者不提供子网和网络掩码。", quiet);
                ret = 0;
            }
            if (f[3].value == "" && f[2].value != "") {
                ferror.set(f[3], "必须提供子网和网络掩码，或者不提供子网和网络掩码。", quiet);
                ret = 0;
            }
            if (f[4].checked && (f[2].value == "" || f[3].value == "")) {
                ferror.set(f[4], "如果未提供路线，则无法推送路线。 请提供子网/网络掩码。", quiet);
                ret = 0;
            }
            return ret;
        }
        ...
        CCDGrid.prototype.dataToView = function(data) {
            var c2c = false;
            for (i = 0; i < tabs.length; ++i) {
                if (ccdTables[i] == this && E('_f_vpn_server' + (i + 1) + '_c2c').checked) c2c = true;
            }

            var temp = ['<input type=\'checkbox\' style="opacity:1" disabled' + (data[0] != 0 ? ' checked': '') + '>', data[1], data[2], data[3], c2c ? '<input type=\'checkbox\' style="opacity:1" disabled' + (data[4] != 0 ? ' checked': '') + '>': 'N/A'];

            var v = [];
            for (var i = 0; i < temp.length; ++i) v.push(i == 0 || i == 4 ? temp[i] : escapeHTML('' + temp[i]));
        ...
        CCDGrid.prototype.reDraw = function() {
            ...
            for (i = 0; i < data.length; ++i) {
                ...
                for (j = 0; j < view.length; ++j) this.tb.rows[i + header].cells[j].innerHTML = view[j];
            }
        }
        ...
        UsersGrid.prototype.verifyFields = function(row, quiet) {
            ...
            // Verify fields in this row of the table
            if (f[1].value == "") {
                ferror.set(f[1], "用户名是必需的。", quiet);
                ret = 0;
            }
            if (f[1].value.indexOf('>') >= 0 || f[1].value.indexOf('<') >= 0) {
                ferror.set(f[1], "用户名不能包含 '<' 或 '>' 字符。", quiet);
                ret = 0;
            }
            if (f[2].value == "") {
                ferror.set(f[2], "密码是必需的。", quiet);
                ret = 0;
            }
            if (f[2].value.indexOf('>') >= 0 || f[1].value.indexOf('<') >= 0) {
                ferror.set(f[2], "密码不能包含 '<' 或 '>' 字符。", quiet);
                ret = 0;
            }
            return ret;
        }
        ...
        function save() {
            ...
            for (i = 0; i < tabs.length; ++i) {
                ...
                if (E('_f_vpn_' + t + '_eas').checked) E('vpn_server_eas').value += '' + (i + 1) + ',';

                if (E('_f_vpn_' + t + '_dns').checked) E('vpn_server_dns').value += '' + (i + 1) + ',';

                var data = ccdTables[i].getAllData();
                var ccd = '';

                for (j = 0; j < data.length; ++j) ccd += data[j].join('<') + '>';
                var userdata = usersTables[i].getAllData();
                var users = '';
                for (j = 0; j < userdata.length; ++j) users += userdata[j].join('<') + '>';
        ...
        function init() {
            ...
            for (i = 0; i < tabs.length; ++i) {
                ...
                ccdTables[i].init('table_' + t + '_ccd', 'sort', 0, [{
                    type: 'checkbox'
                },
                {
                    type: 'text'
                ...
                ccdTables[i].headerSet(['启用', '公用名', '子网', '掩码', '推送']);
                ...
                if (ccdVal.length) {
                    ...
                    for (var j = 0; j < s.length; ++j) {
                        ...
                        var row = s[j].split('<');
                        if (row.length == 5) ccdTables[i].insertData( - 1, row);
                    }
                }
                ...
                usersTables[i].headerSet(['启用', '账号', '密码']);
                ...
                if (usersVal.length) {
                    ...
                    for (var j = 0; j < s.length; ++j) {
                        if (!s[j].length) continue;
                        var row = s[j].split('<');
                        if (row.length == 3) usersTables[i].insertData( - 1, row);
                    }
                }
        ...
        <div class="section" id="htmlConf">
            <script type="text/javascript">
                ...
                for (i = 0; i < tabs.length; ++i) {
                    ...
                    var action = (eval('vpn' + (i + 1) + 'up') ? 'title="停止VPN服务器 ' + (i + 1) + '"><i class="icon-stop"></i>': 'title="开启VPN服务器 ' + (i + 1) + '"><i class="icon-play"></i>');
                    var status = (!eval('vpn' + (i + 1) + 'up') ? '<small style="color: red">(未运行)</small>': '<small style="color: green;">(运行中)</small>');
                    htmlOut += '</ul>' + '<div class="box"><div class="heading">VPN 服务器 #' + (i + 1) + status + ' <a id="_vpn' + t + '_button" class="pull-right" href="#" data-toggle="tooltip"' + 'onclick="toggle(\'vpn' + t + '\', vpn' + (i + 1) + 'up); return false;"' + action + '</a></div><div class="content">';
                    htmlOut += '<div id=\'' + t + '-basic\'>';
                    htmlOut += createFormFields([{
                        title: '连接互联网后运行',
                        name: 'f_vpn_' + t + '_eas',
                        type: 'checkbox',
                        value: nvram.vpn_server_eas.indexOf('' + (i + 1)) >= 0
                    },
                    {
                        title: '接口类型',
                        name: 'vpn_' + t + '_if',
                        type: 'select',
                        options: [['tap', 'TAP'], ['tun', 'TUN']],
                        value: eval('nvram.vpn_' + t + '_if')
                    },
                    {
                        title: '绑定TAP至',
                        indent: 2,
                        name: 'vpn_' + t + '_br',
                        type: 'select',
                        options: [['br0', 'LAN (LAN0)*'], ['br1', 'LAN1 (LAN1)'], ['br2', 'LAN2 (LAN2)'], ['br3', 'LAN3 (LAN3)']],
                        value: eval('nvram.vpn_' + t + '_br'),
                        suffix: ' <small>* default</small> '
                    },
                    {
                        title: '协议',
                        name: 'vpn_' + t + '_proto',
                        type: 'select',
                        options: [['udp', 'UDP'], ['tcp-server', 'TCP']],
                        value: eval('nvram.vpn_' + t + '_proto')
                    },
                    {
                        title: '端口',
                        name: 'vpn_' + t + '_port',
                        type: 'text',
                        value: eval('nvram.vpn_' + t + '_port')
                    },
                    {
                        title: '防火墙',
                        name: 'vpn_' + t + '_firewall',
                        type: 'select',
                        options: [['auto', '自动'], ['external', '仅限外部'], ['custom', '自定义']],
                        value: eval('nvram.vpn_' + t + '_firewall')
                    },
                    {
                        title: '验证模式',
                        name: 'vpn_' + t + '_crypt',
                        type: 'select',
                        options: [['tls', 'TLS'], ['secret', '静态密钥'], ['custom', '自定义']],
                        value: eval('nvram.vpn_' + t + '_crypt'),
                        suffix: '<span id=\'' + t + '_custom_crypto_text\'>&nbsp;<small>(必须手动配置...)</small></span>'
                    },
                    {
                        title: '额外HMAC授权（tls-auth）',
                        name: 'vpn_' + t + '_hmac',
                        type: 'select',
                        options: [[ - 1, '禁用'], [2, '双向'], [0, '入站 (0)'], [1, '出站 (1)']],
                    ...
                    },
                    {
                        ...
                        multi: [{
                            name: 'f_vpn_' + t + '_dhcp',
                            type: 'checkbox',
                            value: eval('nvram.vpn_' + t + '_dhcp') != 0,
                            suffix: ' DHCP '
                        },
                        {
                            name: 'vpn_' + t + '_r1',
                            ...
                            maxlen: 15,
                            size: 17,
                            value: eval('nvram.vpn_' + t + '_r1'),
                            prefix: '<span id=\'' + t + '_range\'>',
                            suffix: '-'
                        },
                        {
                            name: 'vpn_' + t + '_r2',
                    ...
                    },
                    {
                        ...
                        multi: [{
                            ...
                            maxlen: 15,
                            size: 17,
                            value: eval('nvram.vpn_' + t + '_local')
                        },
                        {
                            name: 'vpn_' + t + '_remote',
                            type: 'text',
                            maxlen: 15,
                            size: 17,
                            value: eval('nvram.vpn_' + t + '_remote')
                        }]
                    }]);
                    htmlOut += '</div>';
                    ...
                    htmlOut += createFormFields([{
                        ...
                        maxlen: 4,
                        size: 5,
                        value: eval('nvram.vpn_' + t + '_poll'),
                        suffix: '&nbsp;<small>(分钟，0禁用)</small>'
                    },
                    {
                        title: '将LAN推送到客户端',
                        name: 'f_vpn_' + t + '_plan',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_plan') != 0
                    },
                    {
                        title: '直连客户端<br>重定向到互联网流量',
                        name: 'f_vpn_' + t + '_rgw',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_rgw') != 0
                    },
                    {
                        title: '响应DNS',
                        name: 'f_vpn_' + t + '_dns',
                        type: 'checkbox',
                        value: nvram.vpn_server_dns.indexOf('' + (i + 1)) >= 0
                    },
                    {
                        title: '向客户端广播DNS',
                        name: 'f_vpn_' + t + '_pdns',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_pdns') != 0
                    },
                    {
                        title: '加密方式',
                        name: 'vpn_' + t + '_cipher',
                        type: 'select',
                        options: ciphers,
                        value: eval('nvram.vpn_' + t + '_cipher')
                    },
                    {
                        title: '启用压缩',
                        name: 'vpn_' + t + '_comp',
                        type: 'select',
                        options: [['-1', '禁用'], ['no', '无'], ['yes', '启用'], ['adaptive', '自适应']],
                    ...
                    },
                    {
                        ...
                        maxlen: 10,
                        size: 7,
                        value: eval('nvram.vpn_' + t + '_reneg'),
                        suffix: '&nbsp;<small>(分钟, -1 默认)</small>'
                    },
                    {
                        title: '管理客户端特定选项',
                        name: 'f_vpn_' + t + '_ccd',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_ccd') != 0
                    },
                    {
                        title: '允许客户端< - >客户端通信',
                        name: 'f_vpn_' + t + '_c2c',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_c2c') != 0
                    },
                    {
                        title: '仅允许这些客户端',
                        name: 'f_vpn_' + t + '_ccd_excl',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_ccd_excl') != 0
                    },
                    {
                        title: '',
                        suffix: '<table class=\'line-table\' id=\'table_' + t + '_ccd\'></table>'
                    },
                    {
                        title: '允许用户/密码验证',
                        name: 'f_vpn_' + t + '_userpass',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_userpass') != 0
                    },
                    {
                        title: '仅允许通过验证的用户（无证书）',
                        name: 'f_vpn_' + t + '_nocert',
                        type: 'checkbox',
                        value: eval('nvram.vpn_' + t + '_nocert') != 0
                    },
                    {
                        title: '',
                        suffix: '<table class=\'line-table\' id=\'table_' + t + '_users\'></table>'
                    },
                    {
                        title: '自定义设置',
                        name: 'vpn_' + t + '_custom',
                        type: 'textarea',
                        value: eval('nvram.vpn_' + t + '_custom'),
                        style: 'width: 100%; height: 80px;'
                    }]);
                    htmlOut += '</div>';
                    htmlOut += '<div id=\'' + t + '-keys\' class="langrid">';
                    htmlOut += '<p class=\'keyhelp\'>有关生成密钥的帮助，请参阅OpenVPN <a id=\'' + t + '-keyhelp\'>HOWTO</a>.</p>';
                    htmlOut += createFormFields([{
                        title: '静态密钥',
                        name: 'vpn_' + t + '_static',
                        type: 'textarea',
                        value: eval('nvram.vpn_' + t + '_static'),
                        style: 'width: 100%; height: 80px;'
                    },
                    {
                        title: '证书颁发机构',
                    ...
                    },
                    {
                        ...
                        name: 'vpn_' + t + '_dh',
                        type: 'textarea',
                        value: eval('nvram.vpn_' + t + '_dh'),
                        style: 'width: 100%; height: 80px;'
                    }]);
                    htmlOut += '</div>';
                    htmlOut += '<div id=\'' + t + '-status\'>';
                    htmlOut += '<div id=\'' + t + '-no-status\'><p>服务器未运行或无法读取状态。</p></div>';
                    htmlOut += '<div id=\'' + t + '-status-content\' style=\'display:none\' class=\'status-content\'>';
                    htmlOut += '<div id=\'' + t + '-status-header\' class=\'status-header\'><p>当前数据 <span id=\'' + t + '-status-time\'></span>.</p></div>';
                    ...
                    htmlOut += '<div id=\'' + t + '-status-routing\'><div class=\'section-title\'>路由表</div><table class=\'line-table\' id=\'' + t + '-status-routing-table\'></table><br></div>';
                    htmlOut += '<div id=\'' + t + '-status-stats\'><div class=\'section-title\'>一般统计</div><table class=\'line-table\' id=\'' + t + '-status-stats-table\'></table><br></div>';
                    htmlOut += '<div id=\'' + t + '-status-errors\' class=\'error\'></div>';
                    htmlOut += '</div>';
                    htmlOut += '<div><a href=\'javascript:updateStatus(' + i + ')\'>刷新状态</a></div>';
                    htmlOut += '</div>';
                    htmlOut += '</div></div></div>';
                }
