...
[16,'eDonkey'],[32,'Gnutella'],[64,'Kazaa'],[128,'Mute'],[4096,'PPLive/UUSee'],[256,'SoulSeek'],[512,'Waste'],[1024,'WinMX']
		...
		cg.verifyFields = function(row, quiet) {
			...
			ferror.set(f, '无效的MAC地址或IP地址/范围', quiet);
			return false;
		}
		...
		bpg.dataToView = function(data) {
			...
			s = '';
			if (data[5] != 0) s = ((data[5] == 1) ? ' 目标  ' : ' 源 ') + data[6] + ', ';

			if (data[0] <= -2) s += (s.length ? '任' : '任') + '何协议';
		...
		bpg.setup = function() {
			...
			protos = [[-2, '任何协议'],[-1,'TCP/UDP'],[6,'TCP'],[17,'UDP']];
			for (i = 0; i < 256; ++i) {
			...
			this.init('res-bp-grid', 'sort', 500, [ { multi: [
				...
				{ type: 'select',
					options: [['a','任何端口'],['d','目标端口'],['s','源端口'],['x','源/目标端口']] },
				{ type: 'text', maxlen: 32 },
				{ type: 'select', options: ipp2p },
				{ type: 'select', options: layer7 },
				{ type: 'select',
					options: [[0,'任何地址'],[1,'目标地址'],[2,'源地址']] },
				{ type: 'text', maxlen: 64 }
			] } ] );
			this.headerSet(['规则']);
			this.showNewEditor();
		...
		function save()
		{
			...
			if (data.length >= 8192) {
				alert('这个规则太大了。请减少到 ' + (data.length - 8192) + ' 个字符.');
				return;
			}
...
{ title: '类型', name: 'f_type', id: 'rt_norm', type: 'radio', suffix: ' 正常访问限制', value: (rule[5] != '~') },
{ title: '', name: 'f_type', id: 'rt_wl', type: 'radio', suffix: ' 禁用无线', value: (rule[5] == '~') },
{ title: '适用于', name: 'f_comp_all', type: 'select', options: [[0,'所有网络设备'],[1,'以下设备'],[2,'这些设备以外']], value: 0 },
{ title: '&nbsp;', text: '<table class="line-table col-sm-9" id="res-comp-grid"></table>' },
{ title: '禁止访问', name: 'f_block_all', type: 'checkbox', suffix: ' 禁止互联网访问', value: 0 },
{ title: '端口/应用程序', indent: 2, text: '<table class="line-table col-sm-12" id="res-bp-grid"></table>' },
{ title: 'HTTP请求', indent: 2, name: 'f_block_http', type: 'textarea', value: rule[7] },
{ title: 'HTTP请求的文件', indent: 2, multi: [
{ name: 'f_activex', type: 'checkbox', suffix: ' ActiveX插件 (ocx, cab) &nbsp;&nbsp;', value: (rule[8] & 1) },
{ name: 'f_flash', type: 'checkbox', suffix: ' Flash插件 (swf) &nbsp;&nbsp;', value: (rule[8] & 2) },
{ name: 'f_java', type: 'checkbox', suffix: ' Java插件 (class, jar) &nbsp;&nbsp;', value: (rule[8] & 4) } ] }
]);
				</script>

				<button type="button" value="Delete..." id="delete-button" onclick="delRULE();" class="btn btn-danger"><i class="icon-cancel"></i> 删除</button> &nbsp;
				<button type="button" value="Save" id="save-button" onclick="save();" class="btn btn-primary">保存 <i class="icon-check"></i></button>
				<button type="button" value="Cancel" id="cancel-button" onclick="cancel();" class="btn">取消 <i class="icon-disable"></i></button>
