...
<content>
    <style type="text/css">
        #grid .co2, #grid .co3, #grid .co4, #grid .co5, #grid .co6, #grid .co7,
        #grid .co8 { text-align: right; }
    </style>
    ...
    <script type="text/javascript">
        ...
        grid.populate = function() {
            ...
            for (i = 0; i < avgiptraffic.length; ++i) {
                ...
                if (E('_f_shortcuts').checked) {
                    ...
                    h = h + '<a href="javascript:viewIptHistory(' + i + ')" title="查看IP流量历史">[历史]</a>';
                    h = h + '<a href="javascript:addExcludeList(' + i + ')" title="隐藏此IP地址">[隐藏]</a>';
                    h = h + '</small>';
                }

                d = [h, rescale((b[1] / 1024).toFixed(2)).toString(), rescale((b[2] / 1024).toFixed(2)).toString(), b[3].toFixed(0).toString(), b[4].toFixed(0).toString(), b[5].toFixed(0).toString(), b[6].toFixed(0).toString(), b[7].toFixed(0).toString(), b[8].toFixed(0).toString(), b[9].toString(), b[10].toString()];
            ...
            grid.resort();
            grid.recolor();
            grid.footerSet(['总计 ' + ('<small><i>(' + ((hostslisted.length > 0) ? (hostslisted.length + ' 台主机') : '没有数据') + ')</i></small>'), rescale((rx / 1024).toFixed(2)).toString(), rescale((tx / 1024).toFixed(2)).toString(), tcpi.toFixed(0).toString() + '/' + tcpo.toFixed(0).toString(), udpi.toFixed(0).toString() + '/' + udpo.toFixed(0).toString(), icmpi.toFixed(0).toString() + '/' + icmpo.toFixed(0).toString(), tcpconn.toString(), udpconn.toString()]);
        }
        ...
        grid.dataToView = function(data) {
            return [data[0].toString(), data[1].toString(), data[2].toString(), data[3] + '/' + data[4], data[5] + '/' + data[6], data[7] + '/' + data[8], data[9].toString(), data[10].toString()];
        }

        grid.setup = function() {
            this.init('grid', 'sort');
            this.headerSet(['主机', '下行 (bytes/s)', '上行 (bytes/s)', 'TCP 入站/出站 (pkt/s)', 'UDP 入站/出站 (pkt/s)', 'ICMP 入站/出站 (pkt/s)', 'TCP连接数', 'UDP 连接数']);
        }
    ...
    <div class="pull-right refreshier">
        <script type="text/javascript">
            $('.refreshier').html(genStdRefresh(1, 1, 'ref.toggle()'));
        </script>
    </div>
