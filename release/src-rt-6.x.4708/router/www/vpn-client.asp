...
sections = [['basic', '<i class="icon-system"></i> 基本设置'],['advanced', '<i class="icon-shield"></i> 高级设置'],['keys','<i class="icon-lock"></i> 安全设置'],['policy','<i class="icon-globe"></i> 路由策略'],['status','<i class="icon-info"></i> 状态']];
		routingTables = [];
		statusUpdaters = [];
		...
		RouteGrid.prototype.verifyFields = function(row, quiet)
		{
			...
			if (f[2].value == "" ) { ferror.set(f[2], "值是必需的.", quiet); ret = 0; }
			if (f[2].value.indexOf('>') >= 0 || f[2].value.indexOf('<') >= 0) { ferror.set(f[2], "值不能包含 '<' 或 '>' 符号。", quiet); ret = 0; }
			if (f[2].value.indexOf(' ') >= 0 || f[2].value.indexOf(',') >= 0) { ferror.set(f[2], "值不能包含 '空格' 或 ',' 符号。 每个条目只有一个IP或域。", quiet); ret = 0; }
			if (f[2].value.indexOf(' ') >= 0) { ferror.set(f[2], "值不能包含 '-' 符号。 不支持的IP范围。", quiet); ret = 0; }
			return ret;
		}
		...
		function init()
		{
			...
			for (i = 0; i < tabs.length; ++i)
			{
				...
				routingTables[i].init('table_' + t + '_routing','sort', 0,[
					{ type: 'checkbox' },
					{ type: 'select', options: [[1, '从源IP'],[2, '到目标IP到'],[3,'到域']] },
					{ type: 'text' }]);
				routingTables[i].headerSet(['启用', '类型', '值']);
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<script type="text/javascript">
			...
			for (i = 0; i < tabs.length; ++i) {
				...
				var action = (eval('vpn'+(i+1)+'up') ? 'title="停止VPN客户端 ' + (i+1) + '"><i class="icon-stop"></i>' : 'title="启动VPN客户端 ' + (i+1) + '"><i class="icon-play"></i>');
				...
				{ title: '连接互联网后运行', name: 'f_vpn_'+t+'_eas', type: 'checkbox', value: nvram.vpn_client_eas.indexOf(''+(i+1)) >= 0 },
				{ title: '接口类型', name: 'vpn_'+t+'_if', type: 'select', options: [ ['tap','TAP'], ['tun','TUN'] ], value: eval( 'nvram.vpn_'+t+'_if' ) },
				{ title: '绑定TAP至', indent: 2, name: 'vpn_'+t+'_br', type: 'select', options: [
				['br0','LAN (LAN0)*'],
				['br1','LAN1 (LAN1)'],
				['br2','LAN2 (LAN2)'],
				['br3','LAN3 (LAN3)']
				...
				{ title: '验证模式', name: 'vpn_'+t+'_crypt', type: 'select', options: [ ['tls', 'TLS'], ['secret', '静态密钥'], ['custom', '自定义'] ], value: eval( 'nvram.vpn_'+t+'_crypt' ),
				suffix: '<span id=\''+t+'_custom_crypto_text\'>&nbsp;<small>(必须手动配置...)</small></span>' },
				{ title: '用户名/密码验证', name: 'f_vpn_'+t+'_userauth', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_userauth' ) != 0 },
				...
				{ title: '仅用户名验证', indent: 2, name: 'f_vpn_'+t+'_useronly', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_useronly' ) != 0,
				suffix: '<span style="color: red" id=\''+t+'_ca_warn_text\'>&nbsp<small>警告：必须定义证书颁发机构。</small></span>' },
				{ title: '额外HMAC授权（tls-auth）', name: 'vpn_'+t+'_hmac', type: 'select', options: [ [-1, '禁用'], [2, '双向'], [0, '入站 (0)'], [1, '出站 (1)'] ], value: eval( 'nvram.vpn_'+t+'_hmac' ) },
				{ title: '服务器在同一子网上', name: 'f_vpn_'+t+'_bridge', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_bridge' ) != 0,
				suffix: '</span><span style="color: red" id=\''+t+'_bridge_warn_text\'>&nbsp<small>警告：无法桥接不同的子网。 默认为路由模式。</small></span>' },
				{ title: '在隧道上创建NAT', name: 'f_vpn_'+t+'_nat', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_nat' ) != 0,
				suffix: '<span style="font-style: italic" id=\''+t+'_nat_warn_text\'>&nbsp<small>路由必须手动配置。</small></span>' },
				...
				htmlOut += createFormFields([
				{ title: '轮询间隔', name: 'vpn_'+t+'_poll', type: 'text', maxlen: 4, size: 5, value: eval( 'nvram.vpn_'+t+'_poll' ), suffix: '&nbsp;<small>(分钟, 0 禁用)</small>' },
				...
				{ title: '忽略重定向网关<br>（路由未推送）', name: 'f_vpn_'+t+'_nopull', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_nopull' ) != 0 },
				...
				value: eval( 'nvram.vpn_'+t+'_cn' ), prefix: '<span id=\''+t+'_cn\'> 公用名:&nbsp', suffix: '</span>'} ] },
				{ title: '自定义', name: 'vpn_'+t+'_custom', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_custom' ), style: 'width: 100%; height: 80px;' }
				]);
				htmlOut += ('</div>');
				...
				htmlOut += '<div><ul><li><b>类型 -> 从源IP</b> - 例如: "1.2.3.4" 或 "1.2.3.0/24".'
				htmlOut += '<li><b>类型 -> 到目标IP</b> - 例如: "1.2.3.4" 或 "1.2.3.0/24".';
				htmlOut += '<li><b>类型 -> 到域</b> - 例如: "domain.com". 一行一个';
				htmlOut += '</ul></div></div>';
				...
				{ title: '客户端KEY', name: 'vpn_'+t+'_key', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_key' ), style: 'width: 100%; height: 80px;' },
				]);
				htmlOut += ('</div>');
				htmlOut += ('<div id=\''+t+'-status\'>');
				htmlOut += ('<div id=\''+t+'-no-status\'><p>客户端未运行或状态无法读取。</p></div>');
