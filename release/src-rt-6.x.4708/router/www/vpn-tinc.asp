...
<content>
	...
	<script type="text/javascript">
		...
		tabs = [['config', '<i class="icon-system"></i> 设置'],['hosts', '<i class="icon-globe"></i> 主机'],['scripts', '<i class="icon-hammer"></i> 脚本'],['keys', '<i class="icon-lock"></i> 生成密钥'],['status', '<i class="icon-info"></i> 状态']];
		changed = 0;
		...
		th.setup = function() {
			...
			this.headerSet(['连接到', '名称', '地址', '端口', '压缩方式', '子网']);
		...
		th.verifyFields = function(row, quiet) {
			...
			if (f[1].value == "") {
				ferror.set(f[1], "主机名是必需的。", quiet); return 0 ; }
			else {  ferror.clear(f[1]) }

			if (f[0].checked && f[2].value == "") {
				ferror.set(f[2], "选中连接到时必须提供地址。", quiet); return 0 ; }
			...
			if(E('_tinc_devicetype').value == 'tun'){
				if ((!v_subnet(f[5], 1)) && (!v_ip(f[5], 1))) {
					ferror.set(f[5], "子网或IP地址无效。", quiet); return 0 ; }
				else {  ferror.clear(f[5]) }
			}
			else if (E('_tinc_devicetype').value == 'tap'){
				if (f[5].value != '') {
					ferror.set(f[5], "使用TAP接口类型时，子网为空。", quiet); return 0 ; }
				else {  ferror.clear(f[5]) }
			}

			if (E('_host_ed25519_key').value == "") {
				ferror.set(E('_host_ed25519_key'), "Ed25519 公钥是必需的。", quiet); return 0 ; }
		...
		function verifyFields(focused, quiet)
		{
			...
			if (E('_tinc_name').value == "" && E('_f_tinc_wanup').checked) {
				ferror.set(E('_tinc_name'), "选中“启动WAN”时需要使用主机名。", quiet); return 0 ; }
			else {  ferror.clear(E('_tinc_name')) }

			if (E('_tinc_private_ed25519').value == "" && E('_tinc_custom').value == "" && E('_f_tinc_wanup').checked) {
				ferror.set(E('_tinc_private_ed25519'), "Ed25519 选中“启动WAN”时需要私钥。", quiet); return 0 ; }
			...
			if (!hostdefined && E('_f_tinc_wanup').checked) {
				ferror.set(E('_tinc_name'), "主机名 \"" + E('_tinc_name').value + "\" 必须在选中“启动WAN”时在主机区域中定义。", quiet); return 0 ; }
			else {  ferror.clear(E('_tinc_name')) };
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div id="vpn-tinc">
			<script type="text/javascript">
				...
				var action = ((tincup) ? 'title="停止"><i class="icon-stop"></i>' : 'title="开启"><i class="icon-play"></i>');
				var status = ((!tincup) ? '<small style="color: red">(未运行)</small>' : '<small style="color: green;">(运行中)</small>');

				html += '</ul>\
				<div class="box">\
				<div class="heading">Tinc设置<span id="version"></span> ' + status + '\
				...
				html += createFormFields([
					{ title: '连接互联网后运行', name: 'f_tinc_wanup', type: 'checkbox', value: (nvram.tinc_wanup == 1) },
					{ title: '接口类型', name: 'tinc_devicetype', type: 'select', options: [['tun','TUN'],['tap','TAP']], value: nvram.tinc_devicetype },
					{ title: '模式', name: 'tinc_mode', type: 'select', options: [['switch','交换'],['hub','中转']], value: nvram.tinc_mode },
					{ title: 'VPN网络掩码', name: 'tinc_vpn_netmask', type: 'text', maxlen: 15, size: 25, value: nvram.tinc_vpn_netmask,  suffix: ' <small>整个VPN网络的网络掩码。</small>' },
					{ title: '主机名称', name: 'tinc_name', type: 'text', maxlen: 30, size: 25, value: nvram.tinc_name, suffix: ' <small>也必须在 \'主机\' 中定义。</small>' },
				...
				html +='<ul>';
				html +='<li><b>连接到</b> - Tinc将尝试建立到主机的连接。 需要地址字段';
				html +='<li><b>名称</b> - 主机名称。 此主机必须有一个条目。';
				html +='<li><b>地址</b> <i>(选项)</i> - 必须解析到可以访问主机的外部IP地址。';
				html +='<li><b>端口</b> <i>(选项)</i> - 主机侦听的端口。 如果为空，则使用默认值（655）。';
				html +='<li><b>压缩方式</b> - 用于UDP数据包的压缩级别。 可能的值为 ';
				html +='0（关闭），1（fast zlib）和高达9（best zlib），10（fast lzo）和11（best lzo）的任何整数。';
				...
				html += createFormFields([
					{ title: '防火墙规则', name: 'tinc_manual_firewall', type: 'select', options: [['0','自动'],['1','附加'],['2','自定义']], value: nvram.tinc_manual_firewall },
					{ title: '防火墙', name: 'tinc_firewall', type: 'textarea', value: nvram.tinc_firewall },
					{ title: 'tinc-up创建', name: 'tinc_manual_tinc_up', type: 'select', options: [['0','自动'],['1','自定义']], value: nvram.tinc_manual_tinc_up },
				...
				html += createFormFields([
					{ title: 'Ed25519 公钥', name: 'ed25519_private_key', type: 'textarea', value: "" },
				...
				html +='<div style="visibility:hidden;text-align:right" id="generateWait">请稍后... <div class="spinner"></div></div>';
				html +='</div>';
				...
				html += '</div>'
				html += '<div style="visibility:hidden;text-align:right" id="statusWait">请稍后... <div class="spinner"></div></div>';
