...
Tomato VLAN GUI Copyright (C) 2011 Augusto Bott http://code.google.com/p/tomato-sdhc-vlan/
...
<content>
    <script type='text/javascript'>
        ...
        function init() {
            verifyFields(null, 1);
            $('.nginx-status').html((!nginxup ? '<small style="color: red;">(未运行)</small>': '<small style="color: green;">(运行中)</small>'));
            $('.nginx-status').after('<a href="#" data-toggle="tooltip" class="pull-right nginx-control" title="' + (nginxup ? '停止 NGINX 服务器': '开启 NGINX 服务器') + '" onclick="toggle(\'nginxfp\', nginxup); return false;" id="_nginxfp_button">' + (nginxup ? '<i class="icon-stop"></i>': '<i class="icon-play"></i>') + '</a>');
        }
    </script>
                    ...
                    <li>
                        ...
                        <li>
                            ...
                            <li>
                                ...
                                您是否手动修改了配置文件？ 勾选此框，将保留更改。
                                <br>
                                <li>
                                    <b>
                                        端口:
                                    </b>
                                    要访问的Web服务器使用的端口。 当端口被其他服务使用时请检查冲突。
                                    <br>
                                    <li>
                                        <b>
                                            服务器名称:
                                        </b>
                                        将出现在Internet浏览器顶部的名称。
                                        <br>
                                        <li>
                                            <b>
                                                WWW根目录:
                                            ...
                                            <li>
                                                ...
                                                /tmp/mnt/HDD/www/ 正如你可以在USB路径中找到的。
                                                <br>
                                                <li>
                                                    <b>
                                                        NGINX自定义配置:
                                                    </b>
                                                    您可以添加其他值到nginx.conf以满足您的需要。
                                                </li>
                                                <li>
                                                    <b>
                                                        优先级:
                                                    </b>
                                                    设置服务优先级高于在路由器上运行的其他进程。
                                                    <br>
                                                    操作系统内核具有优先级 -5.
                                                    <br>
                                                    不要选择比内核使用的值更低。 不要使用服务测试页来调整
                                                    <br>
                                                    服务器性能，它的性能低于最终媒体，其中文件将是
                                                    <br>
                                                    位于， USB闪存盘，硬盘驱动器或SSD。
                                                    <br>
                                                </li>
    ...
    <script type="text/javascript">
        $('.content.config-section').forms([{
            ...
            name: 'f_nginx_enable',
            type: 'checkbox',
            value: nvram.nginx_enable == '1'
        },
        {
            title: '启用PHP脚本',
            name: 'f_nginx_php',
            type: 'checkbox',
            value: nvram.nginx_php == '1'
        },
        {
            title: '运行方式',
            name: 'nginx_user',
            type: 'select',
            options: [['root', 'Root'], ['nobody', 'Nobody']],
            value: nvram.nginx_user
        },
        {
            title: '保留配置文件',
            name: 'f_nginx_keepconf',
            type: 'checkbox',
            value: nvram.nginx_keepconf == '1'
        },
        {
            title: '端口',
            name: 'nginx_port',
            ...
            maxlen: 5,
            size: 7,
            value: fixPort(nvram.nginx_port, 85),
            suffix: '<small> 默认: 85</small>'
        },
        {
            title: '上传文件限制',
            name: 'nginx_upload',
            ...
            maxlen: 5,
            size: 7,
            value: nvram.nginx_upload,
            suffix: '<small> MB</small>'
        },
        {
            title: '允许远程访问',
            name: 'f_nginx_remote',
            type: 'checkbox',
            value: nvram.nginx_remote == '1'
        },
        {
            title: '服务器名称',
            name: 'nginx_fqdn',
        ...
        },
        {
            ...
            suffix: '<small>&nbsp;/index.html / index.htm / index.php</small>'
        },
        {
            title: '优先级',
            name: 'nginx_priority',
            ...
            maxlen: 8,
            size: 3,
            value: nvram.nginx_priority,
            suffix: '<small> 最大: -20, 最小: 19, 默认: 10</small>'
        }]);
        $('.content.config-adv').forms([{
            title: 'HTTP-自定义配置(<a href="http://wiki.nginx.org/Configuration" target="_new">NGINX<i class="icon-info"></i></a>)',
            name: 'nginx_httpcustom',
            type: 'textarea',
            value: nvram.nginx_httpcustom,
            style: 'width: 100%; height: 140px;'
        },
        {
            title: 'SERVER-自定义配置(<a href="http://wiki.nginx.org/Configuration" target="_new">NGINX<i class="icon-info"></i></a>)',
            name: 'nginx_servercustom',
            type: 'textarea',
            value: nvram.nginx_servercustom,
            style: 'width: 100%; height: 140px;'
        },
        {
            title: '自定义配置(<a href="http://wiki.nginx.org/Configuration" target="_new">NGINX<i class="icon-info"></i></a>)',
            name: 'nginx_custom',
            type: 'textarea',
            value: nvram.nginx_custom,
            style: 'width: 100%; height: 140px;'
        },
        {
            title: '自定义配置(<a href="http://php.net/manual/en/ini.php" target="_new">PHP<i class="icon-info"></i></a>)',
            name: 'nginx_phpconf',
            type: 'textarea',
            value: nvram.nginx_phpconf,
            style: 'width: 100%; height: 140px;'
        },
        {
            title: '使用用户配置文件',
            name: 'f_nginx_override',
            type: 'checkbox',
            value: nvram.nginx_override == '1',
            suffix: '<small> 使用用户配置文件，一些GUI设置将被忽略</small>'
        },
        {
            title: '用户配置文件路径',
            name: 'nginx_overridefile',
            type: 'text',
            maxlen: 255,
            size: 40,
            value: nvram.nginx_overridefile
        }]);
    </script>
    <script type='text/javascript'>
        init();
