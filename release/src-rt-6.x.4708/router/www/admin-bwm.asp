		...
		<div class="content">
			...
			<script type="text/javascript">
				...
				$('#_fom').forms([
					...
					{ title: '数据保存路径', multi: [
						{ name: 'f_loc', type: 'select', options: [['','内存 (临时)'],['*nvram','NVRAM存储'],
							/* JFFS2-BEGIN */
							['/jffs/','JFFS2分区'],
							/* JFFS2-END */
							/* CIFS-BEGIN */
					...
					{ title: '每月1日', name: 'rstats_offset', type: 'text', value: nvram.rstats_offset, maxlen: 2, size: 4 },
					{ title: '排除的接口', name: 'rstats_exclude', type: 'text', value: nvram.rstats_exclude, maxlen: 64, size: 50, suffix: '<small>(逗号分隔列表)</small>' }
					], { align: 'left' });
			</script>
					...
					<div class="section" id="backup-section">
						<form>
							<div class="input-append">
								...
								<button name="f_backup_button" id="backup-button" onclick="backupButton(); return false;" value="Backup" class="btn">备份 <i class="icon-download"></i></button>
							</div>
						</form>
					</div>
