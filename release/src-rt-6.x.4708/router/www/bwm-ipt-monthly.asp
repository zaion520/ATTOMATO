...
IP Traffic enhancements Copyright (C) 2011 Augusto Bott http://code.google.com/p/tomato-sdhc-vlan/
...
<content>
    <style type="text/css">
        #monthly-grid .co3, #monthly-grid .co4, #monthly-grid .co5 { text-align:
        right; }
    </style>
    <script type="text/javascript" src="js/bwm-hist.js">
    </script>
    <script type="text/javascript" src="js/bwm-common.js">
    </script>
    ...
    <script type="text/javascript">
        ...
        function redraw() {
            ...
            if (monthly_history.length > 0) {
                ...
                for (i = 0; i < monthly_history.length; ++i) {
                    ...
                    if (E('_f_ignorezeroes').checked) if ((b[2] < 1) || (b[3] < 1)) continue;

                    if (E('_f_begin_date').value.toString() != '0') {
                        if (b[0] < E('_f_begin_date').value) continue;
                    }

                    if (E('_f_end_date').value.toString() != '0') {
                        if (b[0] > E('_f_end_date').value) continue;
                    }
                    ...
                    if ((b[1] == getNetworkAddress(nvram.lan_ipaddr, nvram.lan_netmask)) || (b[1] == getNetworkAddress(nvram.lan1_ipaddr, nvram.lan1_netmask)) || (b[1] == getNetworkAddress(nvram.lan2_ipaddr, nvram.lan2_netmask)) || (b[1] == getNetworkAddress(nvram.lan3_ipaddr, nvram.lan3_netmask))) {
                    ...
                    if (E('_f_shortcuts').checked) {
                        h = h + '<br><small>';
                        h = h + '<a href="javascript:viewQosDetail(' + i + ')" title="查看QoS详细信息">[Qos详细信息]</a>';
                        h = h + '<a href="javascript:viewQosCTrates(' + i + ')" title="查看每个连接的传输速率">[Qos速率]</a>';
                        h = h + '<a href="javascript:viewIptDetail(' + i + ')" title="查看该IP的实时流量">[IP速率]</a>';
                        h = h + '<a href="javascript:addExcludeList(' + i + ')" title="隐藏此IP地址">[隐藏]</a>';
                        h = h + '</small>';
                    }
                    var ymd = getYMD(b[0]);
                ...
                dg.resort();
                dg.recolor();
                dg.footerSet(['总计', ('<small><i>(' + (((hostslisted.length > 0) || (subnetslisted.length > 0)) ? ((hostslisted.length > 0) ? (hostslisted.length + ' 个主机') : '') + (((hostslisted.length > 0) && (subnetslisted.length > 0)) ? ', ': '') + ((subnetslisted.length > 0) ? (subnetslisted.length + ' 个子网') : '') : '没有数据') + ')</i></small>'), rescale(rx), rescale(tx), rescale(rx + tx)]);
            }
        }
        ...
        function viewQosDetail(n) {
            cookie.set('qos_filterip', [monthly_history[n][1]], 1);
        ...
        function viewIptDetail(n) {
            cookie.set('ipt_filterip', [monthly_history[n][1]], 1);
        ...
        dg.sortCompare = function(a, b) {
            ...
            case 1:
                ...
                if (r == 0) r = cmpText(da[col], db[col]);
                break;
            case 2:
                // Download
            case 3:
        ...
        function init_filter_dates() {
            ...
            for (var i = 0; i < dates.length; ++i) {
                ...
                b.options[i + 1] = new Option(ymText(ymd[0], ymd[1]), dates[i], false, ((ymd[0] == d.getFullYear()) && (ymd[1] == d.getMonth())));
                e.options[i + 1] = new Option(ymText(ymd[0], ymd[1]), dates[i], false, ((ymd[0] == d.getFullYear()) && (ymd[1] == d.getMonth())));
            }
        }
        ...
        function update_filter_dates(b) {
            for (var i = 0; i < b.options.length; ++i) {
                if (b.options[i].value.toString() != '0') {
                    ...
                    b.options[i].text = ymText(ymd[0], ymd[1]);
                }
            }
        }
        ...
        function init() {
            ...
            if (nvram.cstats_enable != '1') {
                $('.cstats').before('<div class="alert alert-info">IP流量监控已禁用.</b> <a href="/#admin-iptraffic.asp">启用 &raquo;</a>');
                return;
            }
    ...
    <ul class="nav-tabs">
        <li>
            <a class="ajaxload" href="bwm-ipt-realtime.asp">
        ...
        <li>
            <a class="ajaxload" href="bwm-ipt-24.asp">
                <i class="icon-clock">
                </i>
                最近24小时
            </a>
        ...
        <li>
            <a class="ajaxload" href="bwm-ipt-graphs.asp">
                <i class="icon-graphs">
                </i>
                查看图表
            </a>
        ...
        <li>
            <a class="ajaxload" href="bwm-ipt-details.asp">
                <i class="icon-globe">
                </i>
                传输速率
            </a>
        ...
        <li>
            <a class="ajaxload" href="bwm-ipt-daily.asp">
                <i class="icon-clock">
                </i>
                每日
            </a>
    ...
    <div id="cstats" class="box">
        <div class="heading">
            每月IP流量
            <a class="pull-right" href="#" data-toggle="tooltip" title="重新加载信息" onclick="reloadPage(); return false;">
                <i class="icon-refresh">
                </i>
            </a>
        </div>
        <div class="content">
            <table id="monthly-grid" class="line-table">
            </table>
            <br />
            <h4>
                <a href="javascript:toggleVisibility('options');">
                    选项
    ...
    <a href="javascript:genData()" class="btn btn-primary">
        数据
        <i class="icon-drive">
        </i>
    </a>
    <a href="admin-iptraffic.asp" class="btn btn-danger ajaxload">
        配置
        <i class="icon-tools">
        </i>
    </a>
    <script type="text/javascript">
        ...
        c.push({
            ...
            name: 'f_dafm',
            type: 'select',
            options: [['0', 'yyyy-mm'], ['1', 'mm-yyyy'], ['2', 'mmm, yyyy'], ['3', 'mm.yyyy']]
        });
        c.push({
            title: '单位',
            name: 'f_scale',
            type: 'select',
            options: [['0', 'KB'], ['1', 'MB'], ['2', 'GB']]
        });
        c.push({
            title: '显示子网总计',
            name: 'f_subnet',
            type: 'checkbox',
            suffix: ' <small>(在计算最后一行的总流量时不会考虑)</small>'
        });
        c.push({
            title: '隐藏没有流量的IP',
            name: 'f_ignorezeroes',
