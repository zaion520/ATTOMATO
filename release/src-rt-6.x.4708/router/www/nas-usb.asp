<!-- Tomato GUI USB Support - !!TB For use with Tomato Firmware only.
...
<content>
    ...
    <script type="text/javascript">
        ...
        function umountHost(a, host) {
            ...
            xob.onCompleted = function(text, xml) {
                eval(text);
                if (usb.length == 1) {
                    if (usb[0] != 0) ferror.set(a, '设备正忙。 请确保没有应用程序正在使用它，然后重试。', 0);
                }
                xob = null;
                _forceRefresh();
            }
        ...
        function mountHost(a, host) {
            ...
            xob.onCompleted = function(text, xml) {
                eval(text);
                if (usb.length == 1) {
                    if (usb[0] == 0) ferror.set(a, '无法挂载。 验证设备是否已插入，然后重试。', 0);
                }
                xob = null;
                _forceRefresh();
            }
        ...
        dg.sortCompare = function(a, b) {
            ...
            switch (col) {
            case 1:
                if (ra.type == 'Storage' && ra.type == rb.type) r = cmpInt(ra.host, rb.host);
                else r = cmpText(ra.host, rb.host);
                break;
            default:
        ...
        dg.populate = function() {
            ...
            for (i = list.length - 1; i >= 0; --i) {
                ...
                if (e.type != 'Storage') s = '&nbsp<br><small>&nbsp</small>';
                else {
                    if (xob) s = ((e.is_mounted == 0) ? '未挂载': '已挂载') + '<br><small>请稍后...</small>';
                    else if (e.is_mounted == 0) s = '未挂载<br><small><a href="javascript:mountHost(\'L' + i + '\',\'' + e.host + '\')" title="挂载USB存储设备" id="L' + i + '">[ 挂载 ]</a></small>';
                    else s = '已挂载<br><small><a href="javascript:umountHost(\'L' + i + '\',\'' + e.host + '\')" title="安全删除存储设备" id="L' + i + '">[ 卸载 ]</a></small>';
                }
                ...
                if (e.discs) {
                    for (j = 0; j <= e.discs.length - 1; ++j) {
                        ...
                        for (k = 0; k <= parts.length - 1; ++k) {
                            p = parts[k];
                            if (p) {
                                desc = desc + '<br>分区\'' + p[0] + '\'' + (p[3] != '' ? ' ' + p[3] : '') + ((p[5] != 0) ? ' (' + doScaleSize(p[5], 0) + ((p[1] == 1) ? ' / ' + doScaleSize(p[6], 0) + '空闲': '') + ')': '') + '分区' + ((p[1] != 0) ? '': '没有') + ((p[3] == 'swap') ? '激活': '挂载') + ((p[2] != '') ? '于' + p[2] : '');
                            }
                        }
                    }
                }
    ...
    <form id="_fom" method="post" action="tomato.cgi">
                ...
                <script type="text/javascript">
                    ...
                    },
                    {
                        ...
                        multi: [{
                            ...
                            name: 'f_ohci',
                            type: 'checkbox',
                            value: nvram.usb_ohci == 1
                        },
                        {
                            suffix: '&nbsp; UHCI &nbsp;',
                            name: 'f_uhci',
                            type: 'checkbox',
                            value: nvram.usb_uhci == 1
                        }]
                    },
                    null, {
                        title: 'USB 打印支持',
                    ...
                    },
                    {
                        ...
                        name: 'f_bprint',
                        type: 'checkbox',
                        value: nvram.usb_printer_bidirect == 1
                    },
                    null, {
                        title: 'USB存储支持',
                        name: 'f_storage',
                        type: 'checkbox',
                        value: nvram.usb_storage == 1
                    },
                    {
                        title: '文件系统支持',
                        indent: 2,
                        multi: [{
                            suffix: '&nbsp; Ext2 / Ext3 / Ext4 &nbsp;&nbsp;&nbsp;',
                            name: 'f_ext4',
                            type: 'checkbox',
                            value: nvram.usb_fs_ext4 == 1
                        },
                        /* NTFS-BEGIN */
                        {
                            suffix: '&nbsp; NTFS &nbsp;&nbsp;&nbsp;',
                        ...
                        /* NTFS-END */
                        {
                            ...
                            name: 'f_fat',
                            type: 'checkbox',
                            value: nvram.usb_fs_fat == 1
                        },
                        {
                            suffix: '&nbsp; exFAT &nbsp;',
                            name: 'f_exfat',
                            type: 'checkbox',
                            value: nvram.usb_fs_exfat == 1
                        }
                        /* HFS-BEGIN */
                        , {
                            suffix: '&nbsp; HFS / HFS+ &nbsp;',
                    ...
                    /* NTFS-BEGIN */
                    {
                        title: 'NTFS 驱动',
                        indent: 2,
                        name: 'usb_ntfs_driver',
                        type: 'select',
                        options: [['ntfs3g', '打开NTFS-3G驱动程序'],
                        /* TUXERA-BEGIN */
                        ['tuxera', 'Tuxera 驱动'],
                        /* TUXERA-END */
                        /* PARAGON-BEGIN */
                    ...
                    /* LINUX26-END */
                    {
                        ...
                        name: 'f_automount',
                        type: 'checkbox',
                        suffix: ' <small>自动将所有分区挂载到<i> /mnt </i>中的子目录。</small>',
                        value: nvram.usb_automount == 1
                    },
                    {
                        title: '挂载后运行',
                        indent: 2,
                        name: 'script_usbmount',
                        type: 'textarea',
                        value: nvram.script_usbmount
                    },
                    {
                        title: '卸载前运行',
                        indent: 2,
                        name: 'script_usbumount',
                        type: 'textarea',
                        value: nvram.script_usbumount
                    },
                    null,
                    /* LINUX26-BEGIN */
                    {
                        ...
                        name: 'f_idle_enable',
                        type: 'checkbox',
                        suffix: ' <small>空闲时停转硬盘。无需使用闪存驱动器。</small>',
                        value: nvram.idle_enable == 1
                    },
                    null, {
                        title: 'USB 3G调制解调器支持',
                        name: 'f_usb_3g',
                        type: 'checkbox',
                        suffix: ' <small>在从USB端口断开3G调制解调器之前，请记住取消选中复选框。 如果调制解调器使用usbserial模块，您必须重新启动，然后拔下调制解调器。</small>',
                        value: nvram.usb_3g == 1
                    },
                    null,
                    /* LINUX26-END */
                    {
                        title: '热插拔脚本<br> <small>（连接或删除任何USB设备时调用）</small>',
                        name: 'script_usbhotplug',
                        type: 'textarea',
                        value: nvram.script_usbhotplug
                    },
                    null, {
        ...
        <div class="box" id="last-box" data-box="attached-devices">
            ...
            <div class="content">
                <table id="dev-grid" class="line-table">
                </table>
            </div>
        </div>
