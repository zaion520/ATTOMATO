<!-- Tomato GUI Copyright (C) 2006-2009 Jonathan Zarate http://www.polarcloud.com/tomato/
    ...
    <script type="text/javascript">
        ...
        function verifyFields(focused, quiet) {
            ...
            if (!v_ip('_pptp_client_srvip', true) && !v_domain('_pptp_client_srvip', true)) {
                ferror.set(E('_pptp_client_srvip'), "服务器地址无效。", quiet);
                ret = 0;
            }
            if (!E('_f_pptp_client_dfltroute').checked && !v_ip('_pptp_client_srvsub', true)) {
                ferror.set(E('_pptp_client_srvsub'), "子网地址无效。", quiet);
                ret = 0;
            }
            if (!E('_f_pptp_client_dfltroute').checked && !v_ip('_pptp_client_srvsubmsk', true)) {
                ferror.set(E('_pptp_client_srvsubmsk'), "子网掩码无效。", quiet);
                ret = 0;
            }
        ...
        <div class="box" id="pptp-client">
            ...
            <script type="text/javascript">
                $('#pptp-client .content').forms([{
                    title: '连接互联网后启动',
                    name: 'f_pptp_client_enable',
                    type: 'checkbox',
                    value: nvram.pptp_client_enable != 0
                },
                {
                    title: '服务器IP地址',
                    name: 'pptp_client_srvip',
                    type: 'text',
                    size: 17,
                    value: nvram.pptp_client_srvip
                },
                {
                    title: '用户名: ',
                    name: 'pptp_client_username',
                    type: 'text',
                    maxlen: 50,
                    size: 54,
                    value: nvram.pptp_client_username
                },
                {
                    title: '密码: ',
                    name: 'pptp_client_passwd',
                    type: 'password',
                    maxlen: 50,
                    size: 54,
                    value: nvram.pptp_client_passwd
                },
                {
                    title: '加密方式',
                    name: 'pptp_client_crypt',
                    type: 'select',
                    value: nvram.pptp_client_crypt,
                    options: [['0', '自动'], ['1', '无'], ['2', '最大值 (仅128位)'], ['3', '需要 (128或40位)']]
                },
                {
                    title: '无状态MPPE连接',
                    name: 'f_pptp_client_stateless',
                    type: 'checkbox',
                    value: nvram.pptp_client_stateless != 0
                },
                {
                    title: '接受DNS配置',
                    name: 'pptp_client_peerdns',
                    type: 'select',
                    options: [[0, '禁用'], [1, '是'], [2, '唯一']],
                    value: nvram.pptp_client_peerdns
                },
                {
                    title: '重定向到互联网流量',
                    name: 'f_pptp_client_dfltroute',
                    type: 'checkbox',
                    value: nvram.pptp_client_dfltroute != 0
                },
                {
                    title: '远程子网/网络掩码',
                    multi: [{
                        name: 'pptp_client_srvsub',
                        type: 'text',
                        maxlen: 15,
                        size: 17,
                        value: nvram.pptp_client_srvsub
                    },
                    {
                        name: 'pptp_client_srvsubmsk',
                        type: 'text',
                        maxlen: 15,
                        size: 17,
                        prefix: ' /&nbsp',
                        value: nvram.pptp_client_srvsubmsk
                    }]
                },
                {
                    title: '在隧道上创建NAT',
                    name: 'f_pptp_client_nat',
                    type: 'checkbox',
                    value: nvram.pptp_client_nat != 0
                },
                {
                    title: 'MTU值',
                    multi: [{
                        name: 'pptp_client_mtuenable',
                        type: 'select',
                        options: [['0', '默认'], ['1', '自定义']],
                        value: nvram.pptp_client_mtuenable
                    },
                    {
                        name: 'pptp_client_mtu',
                        type: 'text',
                        maxlen: 4,
                        size: 6,
                        value: nvram.pptp_client_mtu
                    }]
                },
                {
                    title: 'MRU值',
                    multi: [{
                        name: 'pptp_client_mruenable',
                        type: 'select',
                        options: [['0', '默认'], ['1', '自定义']],
                        value: nvram.pptp_client_mruenable
                    },
                    {
                        name: 'pptp_client_mru',
                        type: 'text',
                        maxlen: 4,
                        size: 6,
                        value: nvram.pptp_client_mru
                    }]
                },
                {
                    title: '自定义配置',
                    name: 'pptp_client_custom',
                    type: 'textarea',
                    value: nvram.pptp_client_custom,
                    style: "width: 100%; height: 80px;"
                }]);
                $('#pptp-client .pptp-client-status').html((!pptpup ? '<small style="color: red;">(未运行)</small>': '<small style="color: green;">(运行中)</small>'));
                $('#pptp-client .pptp-client-status').after('<a href="#" data-toggle="tooltip" class="pull-right pptp-client-control" title="' + (pptpup ? '停止PPTP客户端': '启动PPTP客户端') + '" onclick="toggle(\'pptpclient\', pptpup); return false;" id="_pptpclient_button">' + (pptpup ? '<i class="icon-stop"></i>': '<i class="icon-play"></i>') + '</a>');
            </script>
        </div>
