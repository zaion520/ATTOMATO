...
<content>
	<script type="text/javascript">
		...
		ug.setup = function() {
			...
			this.headerSet(['外部', '<b>内部', '<b>内部地址', '<b>协议', '<b>描述']);
			ug.populate();
		}
		...
		function verifyFields(focused, quiet)
		{
			...
			if ((enable) && (!E('_f_upnp_lan').checked) && (!E('_f_upnp_lan1').checked) && (!E('_f_upnp_lan2').checked) && (!E('_f_upnp_lan3').checked)) {
				if ((E('_f_enable_natpmp').checked) || (E('_f_enable_upnp').checked)) {
					var m = '必须在至少一个LAN口中启用NAT-PMP或UPnP。 您可以继续，但请记住通过自定义配置配置对UPnP服务的访问';
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box">
			...
			<div class="content">
				...
				<div style="width: 100%; text-align: right"><button type="button" value="Delete All" onclick="deleteAll();" id="upnp-delete-all" class="btn btn-danger">全部删除 <i class="icon-cancel"></i></button>
					<button type="button" value="Refresh" onclick="javascript:reloadPage();" class="btn"><i class="icon-refresh"></i> 刷新</button></div>
			</div>
		</div>
		...
		<div class="box" data-box="forward-upnp-settings">
			...
			<script type="text/javascript">
				$('#upnpsettings').forms([
					...
					!!TB - additional miniupnp settings
					REMOVE-END */
					{ title: '自动删除非活动规则', name: 'f_upnp_clean', type: 'checkbox', value: (nvram.upnp_clean == '1') },
...
suffix: ' &nbsp; <small>(当启用时，UPnP的客户机可以只映射到其IP)</small>',
