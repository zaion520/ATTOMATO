	...
	<script type='text/javascript'>
		...
		lg.verifyFields = function(row, quiet) {
			...
			if(this.countBridge(f[0].selectedIndex) > 0) {
				ferror.set(f[0], '无法为网卡添加另一个条目' + f[0].selectedIndex, quiet);
				ok = 0;
			} else {
				ferror.clear(f[0]);
			}
...
//		if(wl_ifaces[uidx][0].indexOf('.') < 0) {
				if (wl_sunit(uidx)<0) {
					...
					/* REMOVE-BEGIN
					 ...
					 ferror.set('_wl'+u+'_security_mode', 'WPA/WPA2在N和AC模式下不支持TKIP加密.', quiet || !ok);
					 ok = 0;
					 }
					...
					if ((wmode == 'sta') || (wmode == 'wet')) {
						...
						if (wlclnt > 1) {
							...
							ferror.set(a, '在无线客户端模式下不支持N-only，请选择自动.', quiet || !ok);
							ok = 0;
						}
					}
					a = E('_wl'+u+'_wpa_psk');
...
//		if(wl_ifaces[uidx][0].indexOf('.') < 0) {
				if (wl_sunit(uidx)<0) {
					...
					if (wl_vis[uidx]._f_wl_wds_0 == 1) {
						...
						if (!b) {
							ferror.set('_f_wl'+u+'_wds_0', 'WDS需要MAC地址.', quiet || !ok);
							ok = 0;
						}
					}
				}
...
// TODO - what's required ? integrate with tomatogrid?
			...
			if ((fom['lan_ifname'].value != 'br0') || (fom['lan_ipaddr'].value == '0.0.0.0') || (!t)) {
				e.innerHTML = '网口br0必须始终定义，并设置有效的IP地址.';
				e.style.visibility = 'visible';
				setTimeout(
					function() {
						e.innerHTML = '';
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="network-wan">
			<div class="heading" id="mwan-title">MultiWAN多拨设置</div>
			<div class="content" id="wan-settings"></div>
				...
				<script type="text/javascript">
					...
					$( '#wan-settings' ).forms([
						...
						{ title: '目标 1', indent: 2, name: 'f_mwan_ckdst_1', type: 'text', maxlen: 30, size: 30, value: ckdst[ 0 ] || '' },
						{ title: '目标 2', indent: 2, name: 'f_mwan_ckdst_2', type: 'text', maxlen: 30, size: 30, value: ckdst[ 1 ] || '' }
					]);
			...
			<script type="text/javascript">
				...
				for ( var uidx = 1; uidx <= maxwan_num; ++uidx ) {
					...
					$( '#wan-settings' ).forms([
						...
						{ title: '负载平衡比重', name: 'wan' + u + '_weight', type: 'text', maxlen: 3, size: 8, value: nvram[ 'wan' + u + '_weight' ], suffix: ' <i>(故障转移：0; 负载均衡：1 - 256)</i>' },
						...
						{ title: 'L2TP服务器', name: 'wan' + u + '_l2tp_server_ip', type: 'text', maxlen: 128, size: 64, value: nvram[ 'wan' + u + '_l2tp_server_ip' ] },
						{ title: '使用DHCP服务', name: 'f_wan'+u+'_pptp_dhcp', type: 'checkbox', value: (nvram['wan'+u+'_pptp_dhcp'] == 1) },
						...
						{
						title: '连接模式', name: 'wan' + u + '_ppp_demand', type: 'select', options: [ [ '1', '按需连接' ], [ '0', '自动连接' ] ],
						...
						{ title: '单线多拨', name: 'f_wan' + u + '_ppp_mlppp', type: 'checkbox', value: (nvram[ 'wan' + u + '_ppp_mlppp' ] == 1) },
						{ title: '路由调制解调器IP', name: 'wan' + u + '_modem_ipaddr',type: 'text',	maxlen: 15,	size  : 17,	suffix: ' <i>(必须在不同的子网到路由器，0.0.0.0 代表禁用)</i>', value : nvram[ 'wan' + u + '_modem_ipaddr' ]	},
						...
						{ title: '桥接WAN端口到主LAN口(LAN0)', name: 'f_wan' + u + '_islan', type: 'checkbox', value: (nvram[ 'wan' + u + '_islan' ] == 1) }
						]);
		...
		<div class="box" data-box="network-lan">
			...
			<script type="text/javascript">
				...
				$('.content.lan-settings').forms([
	                ...
	                /* DNSSEC-BEGIN */
	                { title: '启用 DNSSEC', name: 'f_dnssec_enable', type: 'checkbox', suffix: ' <i>(必须由上游名称服务器支持)</i>', value: (nvram.dnssec_enable == 1) },
	                /* DNSSEC-END */
	                /* DNSCRYPT-BEGIN */
					...
					{ title: 'DNS服务器', indent: 2, name: 'dnscrypt_resolver', type: 'select', options: [/*dnscrypt_resolvers*/],  value: nvram.dnscrypt_resolver, suffix: ' <a href=\'https://github.com/jedisct1/dnscrypt-proxy/blob/master/dnscrypt-resolvers.csv\' target=\'_new\'>DNS详细信息</a>' },
					{ title: 'DNS服务器地址', indent: 2, name: 'dnscrypt_resolver_address', type: 'text', maxlen: 50, size: 25, value: nvram.dnscrypt_resolver_address, suffix: ' <a href=\'https://github.com/jedisct1/dnscrypt-proxy/blob/master/dnscrypt-resolvers.csv\' target=\'_new\'>DNS详细信息</a>' },
		...
		<script type="text/javascript">
			...
			for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
				...
				if (wl_sunit(uidx)<0) {
					...
					htmlOut += '<div class="box" data-box="network_wl' + u +'"><div class="heading">无线';
					...
					f = [
						...
						{ title: '密钥更新间隔', indent: 2, name: 'wl'+u+'_wpa_gtk_rekey', type: 'text', maxlen: 4, size: 6, suffix: ' <i>(秒)</i>',
