...
<content>
	<script type="text/javascript">
		...
		fog.dataToView = function(data) {
			return [(data[0] != '0') ? '<i class="icon-check icon-green"></i>' : '<i class="icon-cancel icon-red"></i>', ['TCP', 'UDP', 'TCP/UDP'][data[1] - 1], (data[2].match(/(.+)-(.+)/)) ? (RegExp.$1 + ' -<br>' + RegExp.$2) : data[2], data[3], data[4], data[5]];
		}
		...
		fog.setup = function() {
			this.init('fo-grid', 'sort', 100, [
				...
				{ type: 'text', maxlen: 32 }]);
			this.headerSet(['状态', '协议', '源地址', '目标地址', '目标端口', '描述']);
