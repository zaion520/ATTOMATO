<!-- Tomato GUI Samba Server - !!TB For use with Tomato Firmware only.
...
<content>
    <script type="text/javascript">
        ...
        ssg.verifyFields = function(row, quiet) {
            ...
            if (s.length > 0) {
                if (s.search(/^[ a-zA-Z0-9_\-\$]+$/) == -1) {
                    ferror.set(f[0], '共享名称无效。 只允许使用字符“$ A-Z 0-9 - _”和空格。', quiet);
                    return 0;
                }
                if (this.existName(s)) {
                ...
                ferror.set(f[0], '不允许使用空的共享名称。', quiet);
                return 0;
            }
        ...
        ssg.setup = function() {
            ...
            this.headerSet(['共享名称', '目录', '描述', '权限', '隐藏']);
        ...
        function verifyFields(focused, quiet) {
            ...
            if (a == 2) {
                ...
                b = E('_smbd_user');
                if (b.value == 'root') {
                    ferror.set(b, '不允许使用\“root \”账号。', quiet);
                    return 0;
                }
                ferror.clear(b);
            }
        ...
        function save() {
            ...
            if (nvram.wan_wins == '' || nvram.wan_wins == '0.0.0.0') fom.smbd_wins.value = E('_f_smbd_wins').checked ? 1 : 0;
            else fom.smbd_wins.value = nvram.smbd_wins;
    ...
    <form id="_fom" method="post" action="tomato.cgi">
        ...
        <div class="box">
            ...
            <script type="text/javascript">
                ...
                },
                {
                    ...
                    name: 'smbd_cpage',
                    type: 'select',
                    options: [['', '未指定'], ['437', '437 (美国，加拿大)'], ['850', '850 (西欧)'], ['852', '852 (中欧/东欧)'], ['866', '866 (西里尔文/俄文)']
                    /* LINUX26-BEGIN */
                    , ['932', '932 (日语)'], ['936', '936 (简体中文)'], ['949', '949 (韩文)'], ['950', '950 (繁体中文/ Big5)']
                    /* LINUX26-END */
                    ],
                    suffix: ' <small> (启动cmd.exe并键入chcp以查看当前代码页)</small>',
                ...
                },
                {
                    ...
                    name: 'smbd_autoshare',
                    type: 'select',
                    options: [['0', '禁用'], ['1', '只读'], ['2', '读 / 写'], ['3', '隐藏, 读 / 写']],
                ...
                },
                {
                    ...
                    multi: [{
                        ...
                        name: 'f_smbd_master',
                        type: 'checkbox',
                        value: nvram.smbd_master == 1
                    },
                    {
                        suffix: '&nbsp; WINS服务器 (如果未在基本/网络页面上定义) &nbsp;',
                        name: 'f_smbd_wins',
                        type: 'checkbox',
                        value: (nvram.smbd_wins == 1) && (nvram.wan_wins == '' || nvram.wan_wins == '0.0.0.0')
                    }]
                }]);
            </script>
        </div>
        <div class="box">
            ...
            <div class="content">
                <table class="line-table" id="ss-grid">
                </table>
                <br />
                <small>
                    当没有指定共享并且自动共享被禁用时,
