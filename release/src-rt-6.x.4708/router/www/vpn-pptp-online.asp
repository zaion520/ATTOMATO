...
<content>
	...
	<script type="text/javascript">
		...
		dg.populate = function() {
			...
			for (i = list.length - 1; i >= 0; --i) {
				e = list[i];
				hangup = '<div id="div_'+e.pid+'"><a href="javascript:disconnect(\'div_'+e.pid + ')" title="断开连接" id="pid_' + e.pid + '">断开连接</a></div>';
				/* REMOVE-BEGIN */
				//		this.insert(-1, e, [
		...
		dg.setup = function() {
			...
			this.headerSet(['接口', '<b>用户名', '在线时间', 'VPN IP地址', '源IP地址', '操作']);
			this.populate();
			this.sort(1);
		}
	...
	<div class="box">
		<div class="heading">PPTP 用户在线列表</div>
		<div class="content">
			<table id="dev-grid" class="line-table"></table><br />
		</div>
	</div>

	<a href="vpn-pptp-server.asp" class="btn btn-danger ajaxload">设置 <i class="icon-tools"></i></a>
