...
<content>
	...
	<script type="text/javascript">
		...
		pg.setup = function() {
			this.init('tp-grid');
			this.headerSet(['序号', '地址', 'RX Bytes', 'TTL', 'RTT (ms)', '+/- (ms)']);
		}
		pg.populate = function()
		{
			...
			for (i = 0; i < buf.length; ++i) {
				if (r = buf[i].match(/^(\d+) bytes from (.+): .*seq=(\d+) ttl=(\d+) time=(\d+\.\d+) ms/)) {
					...
					stats = '数据包: [发送:' + RegExp.$1 + '] [接收:' + RegExp.$2 + '] [丢失:' + RegExp.$3 + '] <br>';
				}
				else if (buf[i].match(/^round.+ (\d+\.\d+)\/(\d+\.\d+)\/(\d+\.\d+)/)) {
					stats = '往返时间: [最小:' + RegExp.$1  + '] [平均:'+ RegExp.$2  +'] [最大:'+ RegExp.$3 + '] (ms)<br>' + stats;
				}
			}
...
{ title: 'Ping 计数', name: 'f_count', type: 'text', maxlen: 2, size: 7, value: '5' },
{ title: '数据包大小', name: 'f_size', type: 'text', maxlen: 5, size: 7, value: '56', suffix: ' <small>(bytes)</small>' }
]);
			</script>

			<div style="visibility:hidden; text-align: right;" id="wait">请稍后... <div class="spinner"></div></div>
