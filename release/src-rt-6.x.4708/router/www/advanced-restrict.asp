...
<content>
	...
	<script type="text/javascript">
		...
		var dowNames = ['星期天', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'];

		var og = new TomatoGrid();
		og.setup = function() {
			this.init('res-over-grid', 'sort');
			this.headerSet(['描述', '时间表']);
			var r = this.footerSet(['<button type="button" value="Add" onclick="TGO(this).addEntry()" id="res-over-add" class="btn btn-danger">添加 <i class="icon-plus"></i></button>']);
			r.cells[0].colSpan = 2;
		}
