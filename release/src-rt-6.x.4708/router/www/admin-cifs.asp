	...
	<script type="text/javascript">
		...
		function verifyFields(focused, quiet)
		{
			...
			for (i = 1; i <= 2; ++i) {
				...
				sec.disabled = b;
				if (!b) {
					if ((!v_nodelim(unc, quiet, '共享路径')) || (!v_nodelim(user, quiet, '用户名')) || (!v_nodelim(pass, quiet, '密码')) ||
						(!v_nodelim(servern, quiet, '网络名称')) ||
						(!v_nodelim(dom, quiet, '域')) || (!v_nodelim(exec, quiet, '挂载后运行'))) return 0;
					if ((!v_length(user, quiet, 1)) || (!v_length(pass, quiet, 1))) return 0;
					unc.value = unc.value.replace(/\//g, '\\');
					if (!unc.value.match(/^\\\\.+\\/)) {
						ferror.set(unc, '无效的共享路径。', quiet);
						return 0;
					}
				}
				else {
		...
		<div class="fluid-gr" id="mounties">
			<script type="text/javascript">
				...
				createFormFields([
					...
					{ title: '网络名称', indent: 2, name: 'f_cifs1_servern', type: 'text', maxlen: 128, size: 48, value: a[6] },
					...
					{ title: '域', indent: 2, name: 'f_cifs1_dom', type: 'text', maxlen: 32, size: 34, value: a[4] },
					{ title: '挂载后运行', indent: 2, name: 'f_cifs1_exec', type: 'text', maxlen: 64, size: 34, value: a[5] },
					{ title: '安全设置', indent: 2, name: 'f_cifs1_sec', type: 'select',
						options: [['','默认 (NTLM)'],['ntlmi','NTLM and packet signing'],['ntlmv2','NTLMv2'],['ntlmv2i','NTLMv2 and packet signing'],['nontlm','No NTLM'],['lanman','LANMAN'],['none','无']],
						value: a[7] },
					{ title: 'CIFS1 占用', indent: 2, text: cifs1.size ? (scaleSize(cifs1.size - cifs1.free) + ' / ' + scaleSize(cifs1.size)) +
						' (<span class="txtcifs1"></span>) <div class="progress small cifs1"><div class="bar"></div></div>' : '(未挂载)' },
				]) + '</div></div>';
				...
				createFormFields([
					...
					{ title: '共享路径(UNC)', indent: 2, name: 'f_cifs2_unc', type: 'text', maxlen: 128, size: 48, value: b[1] },
					{ title: '网络名称', indent: 2, name: 'f_cifs2_servern', type: 'text', maxlen: 128, size: 48, value: b[6] },
					...
					{ title: '域', indent: 2, name: 'f_cifs2_dom', type: 'text', maxlen: 32, size: 34, value: b[4] },
					{ title: '挂载后运行', indent: 2, name: 'f_cifs2_exec', type: 'text', maxlen: 64, size: 34, value: b[5] },
					{ title: '安全设置', indent: 2, name: 'f_cifs2_sec', type: 'select',
						options: [['','默认 (NTLM)'],['ntlmi','NTLM and packet signing'],['ntlmv2','NTLMv2'],['ntlmv2i','NTLMv2 and packet signing'],['nontlm','No NTLM'],['lanman','LANMAN'],['none','None']],
						value: b[7] },
					{ title: 'CIFS2 占用', indent: 2, text: cifs2.size ? (scaleSize(cifs2.size - cifs2.free) + ' / ' + scaleSize(cifs2.size)) +
						' (<span class="txtcifs2"></span>) <div class="progress small cifs2"><div class="bar"></div></div>' : '(未挂载)' }
				]) + '</div></div>';
