...
function AdvancedTomato() {
	...
	if ( typeof nvram.at_update !== "undefined" && nvram.at_update != '' ) {
		...
		if ( n < lastUpdate || n == null ) {
			...
			$updateNotification = $( '<div class="alert alert-info icon"><a href="#" class="close" data-update="' + nvram.at_update.replace( '.', '' ) + '"><i class="icon-cancel"></i></a>\
				<h5>更新可用!</h5>AdvancedTomato 版本 <b>' + nvram.at_update + '</b> 已发布，可供下载。	&nbsp; <a target="_blank" href="https://advancedtomato.com/">请点击这里了解更多</a>。</div>' );
	...
	if ( typeof nvram.tomatoanon_answer !== "undefined" ) {
		...
		if ( nvram.tomatoanon_answer != '1' ) {
			...
			$( '.container' ).prepend( '<div class="alert alert-warning icon"><h5>注意</h5> 你还没有设置 <b>TomatoAnon项目</b> 。请点击链接 <a onclick="loadPage(\'admin-tomatoanon.asp\')" href="#">TomatoAnon项目设置页面</a> 做出选择吧。</div>' );
...
function systemUI() {
	...
	$.ajax(
		...
		var wanstatus = '<a title="返回首页" href="#" onclick="loadPage(\'#status-home.asp\');">' + ( ( stats.wanstatus[ 0 ] == 'Connected' ) ? '<span style="color: green;">' + stats.wanstatus[ 0 ] + '</span>' : stats.wanstatus[ 0 ] ) + '</a>';
...
function data_boxes() {
	...
	$( '[data-box]' ).each( function() {
		...
		var html   = $( '<a class="pull-right" href="#" title="隐藏/显示"><i class="icon-chevron-' + ((status) ? 'down' : 'up') + '"></i></a>' );
	...
	$.ajax( { url: page, async: true, cache: false, timeout: 2950 } )
		...
		.fail( function( jqXHR, textStatus, errorThrown ) {
			...
			$( '.container .ajaxwrap' ).html( '<div class="box"><div class="heading">错误 - ' + jqXHR.status + '</div><div class="content">\
				<p>GUI界面无法与路由器取得通信！<br>这可能和某些服务异常，或者某些文件损坏所导致的。</p>\
				<a href="/">刷新</a> 也行会解决一些问题。</div></div>' ).addClass( 'ajax-animation' );
