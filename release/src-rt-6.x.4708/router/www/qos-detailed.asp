...
Filtering/Extensions on this QoS/Connection Details page Copyright (C)
2011 Augusto Bott http://code.google.com/p/tomato-sdhc-vlan/ For use with
Tomato Firmware only. No part of this file may be used without permission.
-->
<title>
    详细视图
</title>
<content>
    <style type="text/css">
        #grid .co1 { text-align: center; width: 30px; } #grid .co2 { width: 180px;
        word-break: break-all; } #grid .co3 { width: 55px; } #grid .co4 { width:
        180px; word-break: break-all; } #grid .co5 { width: 55px; } #grid .co6
        { width: 60px; } #grid .co7 { width: 30px; } #grid .co8 { text-align: right;
        width: 75px; } #grid .co9 { text-align: right; width: 75px; }
    </style>
    ...
    <script type="text/javascript">
        ...
        grid.setup = function() {
            this.init('grid', 'sort');
            this.headerSet(['协议', '源', '源端口', '目标', '目标端口', '分类', '规则', '出站流量', '入站流量']);
        }
        ...
        ref.refresh = function(text) {
            ...
            for (i = 0; i < ctdump.length; ++i) {
                ...
                if (E('_f_excludegw').checked) {
                    if ((b[2] == nvram.lan_ipaddr) || (b[3] == nvram.lan_ipaddr) || (b[2] == nvram.lan1_ipaddr) || (b[3] == nvram.lan1_ipaddr) || (b[2] == nvram.lan2_ipaddr) || (b[3] == nvram.lan2_ipaddr) || (b[2] == nvram.lan3_ipaddr) || (b[3] == nvram.lan3_ipaddr) || (b[2] == '127.0.0.1') || (b[3] == '127.0.0.1')) {
                        continue;
                    }
                }

                if (E('_f_excludebcast').checked) {
                    if ((b[3] == getBroadcastAddress(getNetworkAddress(nvram.lan_ipaddr, nvram.lan_netmask), nvram.lan_netmask)) || (b[3] == getBroadcastAddress(getNetworkAddress(nvram.lan1_ipaddr, nvram.lan1_netmask), nvram.lan1_netmask)) || (b[3] == getBroadcastAddress(getNetworkAddress(nvram.lan2_ipaddr, nvram.lan2_netmask), nvram.lan2_netmask)) || (b[3] == getBroadcastAddress(getNetworkAddress(nvram.lan3_ipaddr, nvram.lan3_netmask), nvram.lan3_netmask)) || (b[3] == '255.255.255.255') || (b[3] == '0.0.0.0')) {
                        continue;
                    }
                }
            ...
            else E('numtotalconn').innerHTML = '<small>(' + numconntotal + ' 连接数)</small>';
        }
        ...
        function init() {
            ...
            if (viewClass != -1) $('#qos-details .heading').html('QOS详细: ' + abc[viewClass] + ' <span id=\'numtotalconn\'></span>');
    ...
    <div class="box" id="qos-details">
        ...
        <div class="content">
            ...
            <div id="sesdivfilters" style="display:none">
            </div>
            <script type="text/javascript">
                var c;
                c = [];
    ...
    <script type="text/javascript">
        $('#qos-details').after(genStdRefresh(1, 1, "ref.toggle()"));
    </script>
    <script type="text/javascript">
        init();
    </script>
