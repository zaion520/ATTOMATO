...
<content>
    <script type="text/javascript" src="js/wireless.jsx?_http_id=<% nv(http_id); %>">
    </script>
    <script type="text/javascript" src="js/interfaces.js?_http_id=<% nv(http_id); %>">
    </script>
    <script type="text/javascript" src="js/status-data.jsx?_http_id=<% nv(http_id); %>">
    </script>
    <script type="text/javascript">
        ...
        auth = {
            'disabled': '-',
            'wep': 'WEP',
            'wpa_personal': 'WPA Personal (PSK)',
            'wpa_enterprise': 'WPA Enterprise',
            'wpa2_personal': 'WPA2 Personal (PSK)',
            'wpa2_enterprise': 'WPA2 Enterprise',
            'wpaX_personal': 'WPA / WPA2 Personal',
            'wpaX_enterprise': 'WPA / WPA2 Enterprise',
            'radius': 'Radius'
        };
        enc = {
            'tkip': 'TKIP',
    ...
    <script type="text/javascript">
        ...
        for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
            /* REMOVE-BEGIN
			 ...
			 REMOVE-END */
            if (wl_sunit(uidx) < 0) show_radio.push((nvram['wl' + wl_fface(uidx) + '_radio'] == '1'));
        }

        nphy = features('11n');

        function dhcpc(what, wan_prefix) {
            form.submitHidden('dhcpc.cgi', {
                exec: what,
                prefix: wan_prefix,
                _redirect: '/#status-home.asp'
            });
        }
        ...
        function wlenable(uidx, n) {
            form.submitHidden('wlradio.cgi', {
                enable: '' + n,
                _nextpage: 'status-overview.asp',
                _nextwait: n ? 6 : 3,
                _wl_unit: wl_unit(uidx)
            });
        }
        ...
        function ethstates() {
            ...
            for (uidx = 1; uidx <= nvram.mwan_num; ++uidx) {
                ...
                if ((nvram['wan' + u + '_sta'] == '') && (nvram['wan' + u + '_proto'] != 'lte') && (nvram['wan' + u + '_proto'] != 'ppp3g')) {
                    names['port' + v] = 'WAN' + u; ++v;
                }
            }

            for (uidx = v; uidx <= 4; ++uidx) {
                names['port' + uidx] = 'LAN' + uidx;
            }
        ...
        function earlyInit() {
            ...
            for (uidx = 0; uidx < wl_ifaces.length; ++uidx) {
                if (wl_sunit(uidx) < 0) elem.display('b_wl' + uidx + '_enable', 'b_wl' + uidx + '_disable', show_radio[uidx]);
            }
    ...
    <div class="fluid-grid">
            ...
            <div class="content" id="sesdiv_system">
                ...
                <script type="text/javascript">
                    ...
                    },
                    {
                        title: 'CPU频率',
                        text: stats.cpumhz,
                        suffix: ' <small>(双核)</small>'
                    },
                    {
                        title: '存储空间',
                        text: stats.flashsize
                    },
                    null, {
                        title: '系统时间',
                        rid: 'time',
                    ...
                    {
                        title: 'CPU负载 <small>(1 / 5 / 15 分钟)</small>',
                        rid: 'cpu',
                        text: stats.cpuload
                    },
                    {
                        title: '内存占用',
                        rid: 'memory',
                        text: stats.memory + '<div class="progress small"><div class="bar" style="width: ' + stats.memoryperc + ';"></div></div>'
                    },
                    {
                        title: '交换占用',
                        rid: 'swap',
                        text: stats.swap + '<div class="progress small"><div class="bar" style="width: ' + stats.swapperc + ';"></div></div>',
                        hidden: (stats.swap == '')
                    },
                    {
                        title: 'NVRAM占用',
                        text: scaleSize(nvstat.size - nvstat.free) + ' <small>/</small> ' + scaleSize(nvstat.size) + ' (' + (a).toFixed(2) + '%) <div class="progress small"><div class="bar" style="width: ' + (a).toFixed(2) + '%;"></div></div>'
                    },
                    null, {
                        title: 'CPU温度',
                        rid: 'temps',
        ...
        <script type="text/javascript">
            for (uidx = 1; uidx <= nvram.mwan_num; ++uidx) {
                ...
                var u = (uidx > 1) ? uidx: '';
                $('.MultiWAN').append('<div class="box" id="wan-title' + u + '" data-box="home_wanbox' + u + '"><div class="heading">WAN ' + u + '</div>' + '<div class="content" id="sesdiv_wan' + u + '"></div></div>');

                createFieldTable('', [{
                    title: 'MAC地址',
                    text: nvram['wan' + u + '_hwaddr']
                },
                {
                    title: '连接类型',
                    text: {
                ...
                },
                {
                    title: 'IP地址',
                    rid: 'wan' + u + 'ip',
                    text: stats.wanip[uidx - 1]
                },
                {
                    title: '子网掩码',
                    rid: 'wan' + u + 'netmask',
                    text: stats.wannetmask[uidx - 1]
                },
                {
                    title: '网关',
                    rid: 'wan' + u + 'gateway',
                    text: stats.wangateway[uidx - 1]
                },
                /* IPV6-BEGIN */
                {
                    title: 'IPv6地址',
                    rid: 'ip6_wan',
                    text: stats.ip6_wan,
                    hidden: (stats.ip6_wan == '')
                },
                /* IPV6-END */
                {
                    title: 'DNS',
                    rid: 'wan' + u + 'dns',
                    text: stats.dns[uidx - 1]
                },
                {
                    title: 'MTU',
                    text: nvram['wan' + u + '_run_mtu']
                },
                null, {
                    title: '状态',
                    rid: 'wan' + u + 'status',
                    text: stats.wanstatus[uidx - 1]
                },
                {
                    title: '连接时间',
                    rid: 'wan' + u + 'uptime',
                    text: stats.wanuptime[uidx - 1]
                },
                {
                    title: '剩余租期',
                    rid: 'wan' + u + 'lease',
                    text: stats.wanlease[uidx - 1],
                    ignore: !show_dhcpc[uidx - 1]
                }], '#sesdiv_wan' + u, 'data-table dataonly');
                $('#sesdiv_wan' + u).append('<br><button type="button" class="btn btn-primary pull-left" onclick="wan_connect(' + uidx + ')" value="Connect" id="b' + u + '_connect" style="display:none;margin-right: 5px;">连接 <i class="icon-reboot"></i></button>' + '<button type="button" class="btn btn-danger pull-left" onclick="wan_disconnect(' + uidx + ')" value="Disconnect" id="b' + u + '_disconnect" style="display:none;margin-right: 5px;">断开 <i class="icon-cancel"></i></button>' + '<div id="b' + u + '_dhcpc" class="btn-group pull-left" style="display:none;"><button type="button" class="btn" onclick="dhcpc(\'renew\', \'wan' + u + '\')" value="Renew">续期</button>' + '<button type="button" class="btn" onclick="dhcpc(\'release\', \'wan' + u + '\')" value="Release">释放</button><div class="clearfix"></div></div>');
            }
        </script>
        <div class="box" id="ethernetPorts" data-box="home_ethports">
            ...
            <div class="content" id="sesdiv_lan-ports">
            </div>
        </div>
        <div class="box" id="LAN-settings" data-box="home_lanbox">
            ...
            <div class="content" id="sesdiv_lan">
                <script type="text/javascript">
                    ...
                    {
                        title: '本地网关',
                        text: nvram.lan_gateway,
                        ignore: nvram.wan_proto != 'disabled'
                    },
                    /* IPV6-BEGIN */
                    {
                        title: 'IPv6地址',
                        rid: 'ip6_lan',
                        text: stats.ip6_lan,
                        hidden: (stats.ip6_lan == '')
                    },
                    {
                        title: 'IPv6链路本地地址',
                        rid: 'ip6_lan_ll',
                        text: stats.ip6_lan_ll,
                        hidden: (stats.ip6_lan_ll == '')
                    },
                    /* IPV6-END */
                    {
                        title: 'DNS',
        ...
        <script type="text/javascript">
            for (var uidx = 0; uidx < wl_ifaces.length; ++uidx) {
                ...
                if (wl_ifaces.length > 0) data += ' (' + wl_display_ifname(uidx) + ')';
                data += '</div>';
                ...
                {
                    title: '无线网络模式',
                    text: bgmo[nvram['wl' + u + '_net_mode']],
                ...
                },
                {
                    title: '信道',
                    rid: 'channel' + uidx,
                    text: stats.channel[uidx],
                    ignore: (wl_sunit(uidx) >= 0)
                },
                {
                    title: '信道带宽',
                    rid: 'nbw' + uidx,
                    text: wlstats[uidx].nbw,
                    ignore: ((!nphy) || (wl_sunit(uidx) >= 0))
                },
                {
                    title: '干扰水平',
                    rid: 'interference' + uidx,
                    text: stats.interference[uidx],
                    hidden: ((stats.interference[uidx] == '') || (wl_sunit(uidx) >= 0))
                },
                {
                    title: '速率',
                    rid: 'rate' + uidx,
                    text: wlstats[uidx].rate,
                    ignore: (wl_sunit(uidx) >= 0)
                },
                {
                    title: 'RSSI',
                    rid: 'rssi' + uidx,
                    text: wlstats[uidx].rssi || '',
                    ignore: ((!wlstats[uidx].client) || (wl_sunit(uidx) >= 0))
                },
                {
                    title: '干扰基数',
                    rid: 'noise' + uidx,
                    text: wlstats[uidx].noise || '',
                    ignore: ((!wlstats[uidx].client) || (wl_sunit(uidx) >= 0))
                },
                {
                    title: '信号质量',
                    rid: 'qual' + uidx,
                    text: stats.qual[uidx] || '',
                    ignore: ((!wlstats[uidx].client) || (wl_sunit(uidx) >= 0))
                }], null, 'data-table dataonly');
                data += '<div class="btn-control-group"><br>';
                data += '<button type="button" class="btn btn-primary" onclick="wlenable(' + uidx + ', 1)" id="b_wl' + uidx + '_enable" value="Enable" style="display:none;">启用 <i class="icon-check"></i></button>';
                data += '<button type="button" class="btn btn-danger" onclick="wlenable(' + uidx + ', 0)" id="b_wl' + uidx + '_disable" value="Disable" style="display:none;">禁用 <i class="icon-disable"></i></button>';
                data += '</div></div></div>';
                $('#LAN-settings').after(data);
            }
        </script>
    </div>
    <div class="clearfix refresher">
    </div>
    <script type="text/javascript">
        earlyInit();
    </script>
