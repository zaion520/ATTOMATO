...
open(F, "/root/advancedtomat/release/src-rt-6.x.4708/router/tor/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/root/advancedtomat/release/src-rt-6.x.4708/router/tor/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/root/advancedtomat/release/src-rt-6.x.4708/router/tor/doc/tor.1.txt") or die;
while (<F>) {
